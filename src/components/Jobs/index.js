...
const Job = () => {
  ...
  const [searchDummy, setSearchDummy] = useState('')
  ...
  const searchInputChanged = event => {
    setSearchDummy(event.target.value)
  }

  const searchButtonClicked = () => {
    setSearchInput(searchDummy)
  }
  ...
  const renderJobData = () => (
    <ul className="all-jobs-items-container">
      {jobData.map(eachJob => (
        <Link
  ...
  useEffect(() => {
    fetchProfileDetails()
    fetchJobDetails()
  }, [jwtToken, searchInput, activeEmploymentTypeIds, activeasalaryRangeIds])
